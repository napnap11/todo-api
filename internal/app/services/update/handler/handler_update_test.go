...
func (mockService) Update(req dto.UpdateRequest) dto.UpdateResponse {
...
func TestHandlerCreateSuccessCase(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", "/v1/update", bytes.NewBuffer(rawReq))
	s := mockService{}
	h := NewHandler(s)
	r.Handle("POST", "/v1/update", h.Update)
	r.ServeHTTP(w, req)
